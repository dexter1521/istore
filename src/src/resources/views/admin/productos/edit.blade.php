...
@section('content')
    ...
    <form action="{{ route('admin.productos.update', $producto) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    @foreach($producto->imagenes as $imagen)
                        <div class="col-md-3 mb-2">
                            <img src="{{ asset('storage/' . $imagen->path) }}" class="img-thumbnail" alt="Imagen producto">
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="mb-3 form-check">
            <input type="checkbox" class="form-check-input" id="mostrar_precio" name="mostrar_precio" value="1" {{ $producto->mostrar_precio ? 'checked' : '' }}>
            <label class="form-check-label" for="mostrar_precio">Mostrar precio en catálogo</label>
        </div>
        <div class="mb-3 form-check">
